...
def test_talib_sma(t_asset):
    featurizer = TALibFeaturizer(
        "ma_low", 
        func=lambda kwargs: talib.SMA(real=kwargs["open"], timeperiod=360), 
        add_to_asset=True
    )
    featurizer.get_features_batch(t_asset)
...
def test_talib_batch_eq_iter_ma(t_asset_small, timeperiod):
    asset = t_asset_small
...
@pytest.mark.parametrize("timeperiod", [2, 100])
def test_talib_batch_eq_iter_sma(t_asset_small, timeperiod):
    asset = t_asset_small
    base_test_featurizer_eq(
        asset, 
        TALibFeaturizer(
            "batch", 
            func=lambda kwargs: talib.SMA(real=kwargs["open"], timeperiod=timeperiod), 
            add_to_asset=True
        ),
        TALibFeaturizer(
            "iter", 
